...
public class TaggedValueInterpreterTest {
  ...
  /** Creates a generic test case using a tagName and comparing the transformation formTerm to toTerm. */
  private void genericTaggedValueTest(String tagName, Term fromTerm, Term toTerm) {
    ...
    MachineTag machineTag =
        MachineTag.newBuilder()
            ...
            .build();
    MetadataRecord mr =
        MetadataRecord.newBuilder().setId("1").setMachineTags(Collections.singletonList(machineTag)).build();
  ...
  public void testCollectionTaggedValues() {
    ...
    genericTaggedValueTest(
        TaggedValuesInterpreter.COLLECTION_TAG_NAME, DwcTerm.collectionCode, GbifInternalTerm.collectionKey);

    genericTaggedValueTest(
        TaggedValuesInterpreter.INSTITUTION_TAG_NAME, DwcTerm.institutionCode, GbifInternalTerm.institutionKey);
