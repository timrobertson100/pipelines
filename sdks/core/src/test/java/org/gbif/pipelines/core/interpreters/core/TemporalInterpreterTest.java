...
public class TemporalInterpreterTest {
  ...
  public void testLikelyIdentified() {
    ...
    assertEquals(OccurrenceIssue.IDENTIFIED_DATE_UNLIKELY.name(), tr.getIssues().getIssueList().iterator().next());
  ...
  public void testLikelyModified() {
    ...
    TemporalInterpreter.interpretTemporal(er, tr);
    assertEquals(1, tr.getIssues().getIssueList().size());
    assertEquals(OccurrenceIssue.MODIFIED_DATE_UNLIKELY.name(), tr.getIssues().getIssueList().iterator().next());
  ...
  public void testLikelyRecorded() {
    ...
    assertEquals(1, tr.getIssues().getIssueList().size());
    assertEquals(OccurrenceIssue.RECORDED_DATE_UNLIKELY.name(), tr.getIssues().getIssueList().iterator().next());
  }
  ...
  public void testNow() {
    ...
    // Sorry for this Thread.sleep, we need to run the v1TemporalInterpreter at least 1 second later until
