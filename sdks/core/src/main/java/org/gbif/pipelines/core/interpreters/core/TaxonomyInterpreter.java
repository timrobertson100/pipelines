 ...
 *
 * <p>The interpretation uses the species match kv store to match the taxonomic fields to an existing specie.
 */
@Slf4j
...
public class TaxonomyInterpreter {
  ...
  private static ParsedName toParsedNameAvro(org.gbif.nameparser.api.ParsedName pn) {
    ...
    Optional.ofNullable(pn.getCode()).ifPresent(code -> builder.setCode(NomCode.valueOf(code.name())));
    Optional.ofNullable(pn.getType()).ifPresent(type -> builder.setType(NameType.valueOf(type.name())));
    Optional.ofNullable(pn.getNotho()).ifPresent(notho -> builder.setNotho(NamePart.valueOf(notho.name())));
    Optional.ofNullable(pn.getRank()).ifPresent(rank -> builder.setRank(Rank.valueOf(rank.name())));
    Optional.ofNullable(pn.getState()).ifPresent(state -> builder.setState(State.valueOf(state.name())));
