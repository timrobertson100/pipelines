...
public class OccurrenceHdfsRecordConverter {
      ...
      dateAsString -> {
        ...
        try {
          ...
          TemporalAccessor temporalAccessor =
              FORMATTER.parseBest(
                  dateAsString, ZonedDateTime::from, LocalDateTime::from, LocalDate::from, YearMonth::from, Year::from);
  ...
  /**
   * Sets the lastInterpreted and lastParsed dates if the new value is greater that the existing one or if it is not
   * set.
   */
  ...
  private static BiConsumer<OccurrenceHdfsRecord, SpecificRecordBase> basicRecordMapper() {
    return (hr, sr) -> {
      ...
      Optional.ofNullable(br.getRecordedByIds())
          .ifPresent(
              uis -> hr.setRecordedbyid(uis.stream().map(AgentIdentifier::getValue).collect(Collectors.toList())));

      Optional.ofNullable(br.getIdentifiedByIds())
          .ifPresent(
              uis -> hr.setIdentifiedbyid(uis.stream().map(AgentIdentifier::getValue).collect(Collectors.toList())));
  ...
  /**
   * The id (the <id> reference in the DWCA meta.xml) is an identifier local to the DWCA, and could only have been used
   * for "un-starring" a DWCA star record. However, we've exposed it as DcTerm.identifier for a long time in our public
   * API v1, so we continue to do this.the id (the <id> reference in the DWCA meta.xml) is an identifier local to the
   * DWCA, and could only have been used for "un-starring" a DWCA star record. However, we've exposed it as
  ...
  /**
   * From a {@link Schema.Field} copies it value into a the {@link OccurrenceHdfsRecord} field using the recognized data
   * type.
   *
  ...
  private static void setHdfsRecordField(
      OccurrenceHdfsRecord occurrenceHdfsRecord, Schema.Field avroField, String fieldName, String value) {
    try {
  ...
  private static BiConsumer<OccurrenceHdfsRecord, SpecificRecordBase> taggedValueMapper() {
    return (hr, sr) -> {
      ...
      Optional.ofNullable(tvr.getTaggedValues().get(GbifInternalTerm.programmeAcronym.qualifiedName()))
      ...
      Optional.ofNullable(tvr.getTaggedValues().get(GbifInternalTerm.institutionKey.qualifiedName()))
